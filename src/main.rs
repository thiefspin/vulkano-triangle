...
use vulkano::render_pass::{Subpass, FramebufferAbstract, RenderPass, Framebuffer};

// Because IntelliJ-Rust seems to have trouble with types from Prelude unless imported explicitly.
// Seems like a regression of https://github.com/intellij-rust/intellij-rust/issues/2877
use std::prelude::v1::*;
use std::sync::Arc;
...
fn main() {
    ...
    println!(
        "Using device: {} (type: {:?})",
        physical.properties().device_name.as_ref().expect("Device name not known."),
        physical.properties().device_type.as_ref().expect("Device type not available.")
    );
